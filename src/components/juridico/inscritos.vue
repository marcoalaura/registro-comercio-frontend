<template>
  <v-card elevation="0">
    <section>
      <small>Bandeja de Inscritos</small>
      <p>&nbsp;</p>
      <h2>Bandeja de Inscritos</h2>
      <p>En esta sección podrá revisar los trámites inscritos por su persona.</p>
    </section>
    <section>
      <crud-table
        ref="crudTable"
...
const urlMisTramites = 'juridico/inscritos';
...
export default {
  ...
  methods: {
    ...
    inscritosVer (item) {
      this.$router.push(`/juridico/inscrito/${item.id}`);
    },
    seguimiento(item) {
      this.codigo = `${item.id}`;
